...
Dos cajeros automáticos. Una cuenta con L5,000. Dos retiros simultáneos de L5,000 cada uno.

Ambos cajeros consultan el saldo. Ambos ven L5,000. Ambos aprueban. La cuenta termina en L-5,000.

Eso es una race condition. Y no necesitas fraude para que ocurra — basta con dos peticiones legítimas llegando al mismo tiempo a un sistema que no fue diseñado para manejarlas juntas.

He revisado sistemas bancarios donde este escenario era posible en producción.

Los errores de concurrencia son los más traicioneros del backend. No aparecen en local. No aparecen en QA. Aparecen bajo carga real, cuando el timing entre hilos es exacto, y el daño ya está hecho antes de que alguien lo detecte.

Cuatro patrones que veo repetirse:

**1. Race condition en verificar-y-modificar.** La solución no está en Java — está en hacer la operación atómica directamente en la BD con un UPDATE condicional.

**2. Contadores compartidos sin sincronización.** `int++` no es atómico. `AtomicInteger.incrementAndGet()` sí. La diferencia es un contador que miente bajo carga.

**3. Deadlock por orden inconsistente de locks.** Si necesitas dos locks, adquiérelos siempre en el mismo orden. Sin excepciones.

**4. Visibilidad entre hilos.** Un hilo puede leer un valor cacheado en CPU que otro hilo ya modificó. `volatile` lo resuelve para casos simples.

La concurrencia no es una optimización que agregas después. Es una decisión de diseño desde el primer día.

¿Has encontrado una race condition en producción? ¿Cómo la detectaron?

---

### X / Twitter (hilo tweet por tweet)

```
Tweet 1:
Dos cajeros. Una cuenta con L5,000.
Dos retiros de L5,000 al mismo tiempo.

Ambos ven el saldo disponible.
Ambos aprueban.

La cuenta termina en -L5,000.

Eso es una race condition.
Y he visto sistemas bancarios donde era posible. 🧵

---

Tweet 2:
Los errores de concurrencia son
los más traicioneros del backend.

No aparecen en local.
No aparecen en QA.

Aparecen bajo carga real,
cuando el timing es exacto,
...
if (cuenta.getSaldo() >= monto) {  // Hilo A: saldo OK
                                    // Hilo B: saldo OK (aún no se debitó)
    cuenta.debitar(monto);          // Hilo A debita
}                                   // Hilo B debita → saldo negativo

Entre verificar y modificar
hay una ventana de tiempo.
Bajo carga, dos hilos caben en esa ventana.

---

Tweet 4:
La solución no está en Java.
...
Tweet 5:
Segundo error: contadores compartidos.

int contador = 0;
contador++; // No es atómico — son 3 instrucciones

Con 500 hilos: el contador miente.

Solución:
AtomicInteger contador = new AtomicInteger(0);
contador.incrementAndGet(); // Atómico garantizado

---

Tweet 6:
Tercer error: deadlock.

Hilo A tiene lock 1, espera lock 2.
Hilo B tiene lock 2, espera lock 1.
Ninguno avanza. El sistema se congela.

Fix: adquiere los locks
...
Un sistema que no fue diseñado
para concurrencia no se arregla
con un synchronized de último momento.

---

Tweet 8:
Artículo 4 de la serie "Errores Comunes en Programación".

Race conditions, deadlocks, visibilidad entre hilos.
Con ejemplos reales de sistemas bancarios:
[link al artículo]

¿Has encontrado una race condition en producción?
¿Cómo la detectaron?
```
